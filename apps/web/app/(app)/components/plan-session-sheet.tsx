...
/** Read-only detail sheet for an AI plan session; open while `item` is set. */
export function PlanSessionSheet({ item, onClose }: { item: PlanItem | null; onClose: () => void }) {
    ...
    <BottomSheet
      ...
      {item && (
        <div className='flex flex-col gap-4'>
          {detailLine && <p className='text-foreground text-sm font-semibold'>{detailLine}</p>}
          {item.description && (
            <p className='text-muted-foreground text-sm leading-relaxed whitespace-pre-line'>{item.description}</p>
          )}
          {item.details.notes && <p className='text-muted-foreground text-sm leading-relaxed'>{item.details.notes}</p>}
          <dl className='flex flex-col'>
            <DetailRow
              label='Type'
              value={planItemTypeLabel(item.itemType)}
              icon={<PlanTypeIcon type={item.itemType} className='text-muted-foreground size-4' />}
            />
            <DetailRow label='Day' value={weekDayOf(item.dayOfWeek)?.name ?? "—"} />
          </dl>
          {videoUrl && (
            <a
              href={videoUrl}
