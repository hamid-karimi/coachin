...
| `POST /training/plans/{id}/checkin` | `{checkinWeek, summary, items}` → one transaction under the profile lock: records the check-in, rewrites **only** the target week (≤ 60 items), +20 XP once (`weekly_checkin:<plan>:<week>`). The scorecard and decision are recomputed here (legacy trusted the posted ones); items are revalidated like generated plans and forced onto the target week, and keep their description (the strength prescription — legacy's rewrite dropped it, so adjusted weeks lost the log sheet prefill); 409 when the week moved on or was already checked in |
